 ...
 * purpose binarizer for uses outside ZXing. This class extends
 * GlobalHistogramBinarizer, using the older histogram approach for 1D readers,
 * and the newer local approach for 2D readers. 1D decoding using a per-row
 * histogram is already inherently local, and only fails for horizontal
 * gradients. We can revisit that problem later, but for now it was not a win to
 * use local blocks for 1D. This Binarizer is the default for the unit tests and
 * the recommended class for library users.
 * 
