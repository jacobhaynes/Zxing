 ...
 * PNG or return a bitmap array. Author: Jacob Haynes
 * (jacob.haynes@colorado.edu), Sergey Ushakov (sergey@verify.kg) Date:
 * 01.11.2004 (Ushakov) Updated: 08.05.2011 (Haynes)
 */
public class PDF417Ext {
    ...
    public void encode() {
        /*
         * Encode procedure is simple as A, B, C, D ..... :) A) High Level
         * Encoding 1. Convert data to codewords using Binary Encode Mode. 2.
         * Determine codeword count. 3. Determine ECC Level, using that codeword
         * count. (if auto detection) 4. Determine symbol size. 5. Make padding
         * codewords, then put lenght of dataCodewords before first codeword. 6.
         * Make ECC calculation 7. Fill out codeword sequence. B) Low level
         * Encoding 1. Initialize pattern array 2. Fill out start patterns, stop
         * patterns, left and right row indicator. 3. Fill out codewords.
         */
        highLevelEncode();
        lowLevelEncode();
    }
    ...
    private void fillRect(byte[][] byteArray, int startX, int startY, int endX, int endY, byte color) {
    ...
    public byte[][] getScaleArray(int ammount) {
        if (ammount > 1) {
            byte[][] output = new byte[bitarray.length * ammount][bitarray[0].length * ammount];
            for (int ii = 0; ii < bitarray.length; ii++) {
                for (int jj = 0; jj < bitarray[0].length; jj++) {
                    byte colorByte = bitarray[ii][jj];
                    fillRect(output, ii * ammount, jj * ammount, (ii + 1) * ammount, (jj + 1)
                            * ammount, colorByte);
                }
            }
            return output;
    ...
    /**
     * High Level Encoding 1. Convert data to codewords using Binary Encode
     * Mode. 2. Determine codeword count. 3. Determine ECC Level, using that
     * codeword count. (if auto detection) 4. Determine symbol size. 5. Make
     * padding codewords, then put lenght of dataCodewords before first
     * codeword. 6. Make ECC calculation 7. Fill out codeword sequence.
     */
    private void highLevelEncode() {
        ...
        System.arraycopy(eccCW.data, 0, codewords, dataCW.data.length, eccCW.data.length);
    }

    /**
     * Low level Encoding 1. Initialize pattern array 2. Fill out start
    ...
    protected int lri(int row) {
        ...
        switch (c) {
            ...
            case 1:
                return 30 * (row / 3) + eccLevel.getEccLevel() * 3 + ((size.getHeight() - 1) % 3);
            case 2:
    ...
    protected int rri(int row) {
        ...
        throw new PDF417EncodeException("Assertion failed while calculating right-row indicator.");
    }
    ...
    /**
     * Changes the height and width of the barcode It determines the number of
     * rows/columns This size will be increased if there is too much data
     */
