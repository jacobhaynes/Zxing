...
import com.google.zxing.pdf417.encoder.External.PDF417Ext;
...
public final class Encoder {
     ...
     * result in "pdf417". We recommend you to use PDF417.EC_LEVEL_L (the lowest
     * level) for "getECLevel" since our primary use is to show QR code on
     * desktop screens. We don't need very strong error correction for this
     * purpose. Note that there is no way to encode bytes in MODE_KANJI. We
     * might want to add EncodeWithMode() with which clients can specify the
     * encoding mode. For now, we don't need the functionality.
     */
    public static void encode(String content, ErrorCorrectionLevel ecLevel, PDF417 pdf417)
            throws WriterException {
        encode(content, ecLevel, null, pdf417);
    }
    
    public static ByteMatrix encode(String content) {
        ...
        return byteMatrixFrombitArray(encoderExt.getScaleArray(4));
    }
    ...
    public static void encode(String content, ErrorCorrectionLevel ecLevel, Hashtable<?, ?> hints,
        ...
        initPDF417(numInputBits, ecLevel, mode, pdf417);
        ...
        appendLengthInfo(numLetters, pdf417.getVersion(), mode, headerAndDataBits);
        headerAndDataBits.appendBitArray(dataBits);

        // Step 5: Terminate the bits properly.
        terminateBits(pdf417.getNumDataBytes(), headerAndDataBits);

        // Step 6: Interleave data bits with error correction code.
        BitArray finalBits = new BitArray();
        interleaveWithECBytes(headerAndDataBits, pdf417.getNumTotalBytes(),
                pdf417.getNumDataBytes(), pdf417.getNumRSBlocks(), finalBits);

        // Step 7: Choose the mask pattern and set to "pdf417".
        ByteMatrix matrix = new ByteMatrix(pdf417.getMatrixWidth(), pdf417.getMatrixWidth());
        ...
        matrix = byteMatrixFrombitArray(encoderExt.getScaleArray(4));

        pdf417.setMatrix(matrix);
        // Step 9. Make sure we have a valid QR Code.
        
       if (!pdf417.isValid()) { throw new
         WriterException("Invalid pdf417: " + pdf417.toString()); }
    ...
    /**
     * Initialize "pdf417" according to "numInputBits", "ecLevel", and "mode".
     * On success, modify "pdf417".
     */
    private static void initPDF417(int numInputBits, ErrorCorrectionLevel ecLevel, Mode mode,
            PDF417 pdf417) throws WriterException {
        pdf417.setECLevel(ecLevel);
        pdf417.setMode(mode);
        ...
        for (int versionNum = 1; versionNum <= 40; versionNum++) {
            ...
            if (numDataBytes >= getTotalInputBytes(numInputBits, version, mode)) {
                ...
                pdf417.setVersion(versionNum);
                pdf417.setNumTotalBytes(numBytes);
                pdf417.setNumDataBytes(numDataBytes);
                pdf417.setNumRSBlocks(numRSBlocks);
                // getNumECBytes = 196 - 66 = 130
                pdf417.setNumECBytes(numEcBytes);
                // matrix width = 21 + 6 * 4 = 45
                pdf417.setMatrixWidth(version.getDimensionForVersion());
                return;
            }
        }
    ...
    static void interleaveWithECBytes(BitArray bits, int numTotalBytes, int numDataBytes,
        ...
        for (int i = 0; i < maxNumDataBytes; ++i) {
            for (int j = 0; j < blocks.size(); ++j) {
                byte[] dataBytes = blocks.elementAt(j).getDataBytes();
        ...
        for (int i = 0; i < maxNumEcBytes; ++i) {
            for (int j = 0; j < blocks.size(); ++j) {
                byte[] ecBytes = blocks.elementAt(j).getErrorCorrectionBytes();
                if (i < ecBytes.length) {
    ...
    private static ByteMatrix byteMatrixFrombitArray(byte[][] input){
        ByteMatrix output = new ByteMatrix(input.length, input[0].length);
        for (int ii = 0; ii < input.length; ii++){
            for (int jj = 0; jj < input[0].length; jj++){
                //Zero is white in the bytematrix
                output.set(ii, jj, ((input[ii][jj] == 0) ? 1 : 0));
            }
        }
        return output;
    }
