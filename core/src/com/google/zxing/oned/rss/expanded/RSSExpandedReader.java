...
public final class RSSExpandedReader extends AbstractRSSReader{
    ...
    private final Vector<ExpandedPair> pairs = new Vector<ExpandedPair>(MAX_PAIRS);
    ...
    Vector<ExpandedPair> decodeRow2pairs(int rowNumber, BitArray row) throws NotFoundException {
        while(true){
    ...
    private static Result constructResult(Vector<ExpandedPair> pairs) throws NotFoundException{
        BitArray binary = BitArrayBuilder.buildBitArray(pairs);

        AbstractExpandedDecoder decoder = AbstractExpandedDecoder.createDecoder(binary);
        String resultingString = decoder.parseInformation();

        ResultPoint [] firstPoints = pairs.elementAt(0).getFinderPattern().getResultPoints();
        ResultPoint [] lastPoints  = pairs.lastElement().getFinderPattern().getResultPoints();
    ...
    private boolean checkChecksum(){
        ExpandedPair firstPair = this.pairs.elementAt(0);
        ...
        for(int i = 1; i < this.pairs.size(); ++i){
            ExpandedPair currentPair = this.pairs.elementAt(i);
    ...
    // not private for testing
    ExpandedPair retrieveNextPair(BitArray row, Vector<ExpandedPair> previousPairs, int rowNumber) throws NotFoundException{
    ...
    private boolean checkPairSequence(Vector<ExpandedPair> previousPairs, FinderPattern pattern) throws NotFoundException{
        ...
        for(int pos = 0; pos < previousPairs.size(); ++pos) {
            this.currentSequence[pos] = previousPairs.elementAt(pos).getFinderPattern().getValue();
        }
    ...
    private void findNextPair(BitArray row, Vector<ExpandedPair> previousPairs, int forcedOffset) throws NotFoundException{
        ...
        if (forcedOffset >= 0) {
            ...
            ExpandedPair lastPair = previousPairs.lastElement();
