 ...
 *
 * @authors Jacob Haynes (jacob.haynes@colorado.edu, jacob.haynes@google.com),
 * 		    dswitkin@google.com (Daniel Switkin)
 */
final class PDF417Encoder {

  private static final String TAG = PDF417Encoder.class.getSimpleName();
  ...
  PDF417Encoder(Activity activity, Intent intent, int dimension) {
  ...
  private boolean encodeContentsFromZXingIntent(Intent intent) {
     // Default to PDF_417 if no format given.
    ...
    if (format == null || BarcodeFormat.PDF_417.equals(format)) {
      String type = intent.getStringExtra(Intents.Encode.TYPE);
      if (type == null || type.length() == 0) {
        return false;
      }
      this.format = BarcodeFormat.PDF_417;
      encodePDF417Contents(intent, type);
    } else {
  ...
  private boolean encodeContentsFromShareIntentDefault(Intent intent) {
    ...
    try {
      ...
      Result result = new Result(vcardString, vcard, null, BarcodeFormat.PDF_417);
      ParsedResult parsedResult = ResultParser.parseResult(result);
      if (!(parsedResult instanceof AddressBookParsedResult)) {
        Log.d(TAG, "Result was not an address");
        return false;
      }
      if (!encodePDF417Contents((AddressBookParsedResult) parsedResult)) {
  ...
  private void encodePDF417Contents(Intent intent, String type) {
  ...
  private boolean encodePDF417Contents(AddressBookParsedResult contact) {
  ...
  Bitmap encodeAsBitmap() throws WriterException {
    ...
    ByteMatrix result = Encoder.encode(contents);
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        pixels[offset + x] = result.get(x, y) == 1 ? BLACK : WHITE;
      }
    }
